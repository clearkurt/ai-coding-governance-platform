...
The separate ignored cancellation integration proves the daemon-required lifecycle: `turn/interrupt` must produce the strict `interrupted` terminal with no approval or workspace changes, then task-bound App Server shutdown must close the active localhost Responses stream. Two consecutive real runs passed, followed by a successful fresh real text turn. The daemon keeps its single-task lease until that terminal is persisted, skips shadow synchronization, rotates the process tree, and prevents synthetic failure from overriding the cancellation; see the audit document.
