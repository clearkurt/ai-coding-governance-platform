...
async fn handle_task(config:&mut Config, envelope:&Envelope) -> Result<serde_json::Value> { if envelope.payload["kind"].as_str()==Some("select_root") { let root_id=envelope.payload["rootId"].as_str().context("缺少 rootId")?; let selected=FileDialog::new().set_title("更换 AI Agent 允许访问的工程目录").pick_folder().context("未选择新目录")?; let path=fs::canonicalize(selected)?; let label={ let root=config.roots.iter_mut().find(|root|root.id==root_id).context("未授权的项目根目录")?; root.path=path; root.label= root.path.file_name().unwrap_or_default().to_string_lossy().to_string(); root.label.clone() }; fs::write(config_path()?,serde_json::to_vec_pretty(config)?)?; return Ok(serde_json::json!({"rootId":root_id,"label":label})); } sandbox::execute(&config.roots,&envelope.payload) }
