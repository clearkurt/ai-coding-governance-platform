...
const cmdTask = await app.inject({ method:'POST', url:`/api/agents/${deviceId}/tasks`, headers:{ cookie }, payload:{ rootId, kind:'run_command', payload:{ command:'npm install', cwd:'', timeoutSeconds:120 } } });
...
ws.send(JSON.stringify({ version:1, messageId:'smoke-cmd-awaiting', type:'task_result', deviceId, taskId: cmdMsg.taskId, payload:{ status:'awaiting_approval', result:{ status:'awaiting_approval', command:'npm install', cwd:'', reason:'需要批准', timeoutSeconds:120 } } }));
...
if (cmdExecMsg.payload.command !== 'npm install') throw new Error('run_command 命令被篡改');
if (cmdExecMsg.payload.timeoutSeconds !== 120) throw new Error('run_command 超时参数未透传');
ws.send(JSON.stringify({ version:1, messageId:'smoke-cmd-result', type:'task_result', deviceId, taskId: cmdExecMsg.taskId, payload:{ status:'completed', result:{ status:'completed', command:'npm install', cwd:'', exitCode:0, stdout:'', stderr:'', durationMs:5, timeoutSeconds:120 } } }));
...
// run_command 拒绝链路
const denyTask = await app.inject({ method:'POST', url:`/api/agents/${deviceId}/tasks`, headers:{ cookie }, payload:{ rootId, kind:'run_command', payload:{ command:'git fetch', cwd:'' } } });
...
ws.send(JSON.stringify({ version:1, messageId:'smoke-cmd-deny', type:'task_result', deviceId, taskId: denyMsg.taskId, payload:{ status:'awaiting_approval', result:{ status:'awaiting_approval', command:'git fetch', cwd:'', reason:'需要批准' } } }));
