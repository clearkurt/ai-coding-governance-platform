...
function sendPendingTasks(deviceId: string) { const socket = agentSockets.get(deviceId); if (!socket) return; const tasks = db.prepare("SELECT * FROM agent_tasks WHERE device_id=? AND status IN ('queued','approved') AND expires_at>? ORDER BY created_at").all(deviceId, now()) as Array<Record<string, unknown>>; for (const task of tasks) socket.send(JSON.stringify({ version: 1, messageId: id(), type: 'task', deviceId, taskId: task.id, payload: JSON.parse(String(task.payload)) })); }
...
app.post('/api/agents/:id/tasks', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const deviceId = (request.params as {id:string}).id; if (!db.prepare('SELECT 1 FROM devices WHERE id=? AND user_id=?').get(deviceId,user.id)) return reply.code(404).send({error:'设备不存在'}); const parsed = z.object({ rootId:z.string().uuid(), kind:z.enum(['list_files','read_file','stage_patch']), payload:z.record(z.unknown()) }).safeParse(request.body); if (!parsed.success) return reply.code(400).send({error:'任务参数无效'}); if (!db.prepare('SELECT 1 FROM device_roots WHERE id=? AND device_id=?').get(parsed.data.rootId,deviceId)) return reply.code(400).send({error:'项目根目录无效'}); const taskId=id(); const expiresAt=new Date(Date.now()+10*60_000).toISOString(); db.prepare('INSERT INTO agent_tasks (id,device_id,user_id,root_id,kind,payload,status,expires_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(taskId,deviceId,user.id,parsed.data.rootId,parsed.data.kind,JSON.stringify({...parsed.data.payload,rootId:parsed.data.rootId,kind:parsed.data.kind}),'queued',expiresAt,now(),now()); taskEvent(taskId,'queued',{}); audit(user.id,'agent_task_created',{taskId,deviceId,kind:parsed.data.kind}); sendPendingTasks(deviceId); return { taskId, status:'queued', expiresAt }; });
...
app.get('/api/agent/ws', { websocket: true }, (socket) => {
  ...
  socket.on('message', (raw: Buffer) => { try { const message=JSON.parse(raw.toString()) as {type:string;payload?:Record<string,unknown>;taskId?:string;deviceId?:string};
    ...
    const p=message.payload ?? {}; const requestedDevice=String(message.deviceId ?? ''); const credential=String(p.credential ?? ''); const device=db.prepare('SELECT * FROM devices WHERE id=? AND credential_hash=?').get(requestedDevice,sha(credential)) as Record<string,unknown>|undefined; if (!device) return socket.close(); deviceId=requestedDevice; agentSockets.set(deviceId,socket); db.prepare("UPDATE devices SET status='online',last_seen_at=?,version=? WHERE id=?").run(now(),String(p.version ?? device.version),deviceId); if (message.type==='hello') { socket.send(JSON.stringify({type:'hello_result',deviceId,payload:{protocol:1}})); sendPendingTasks(deviceId); return; }
    if (message.type==='heartbeat') { db.prepare("UPDATE devices SET status='online',last_seen_at=? WHERE id=?").run(now(),deviceId); return; }
    if (message.type==='task_status' || message.type==='task_result' || message.type==='task_error') { const taskId=String(message.taskId ?? ''); const task=db.prepare('SELECT * FROM agent_tasks WHERE id=? AND device_id=?').get(taskId,deviceId) as Record<string,unknown>|undefined; if (!task) return; const status=String(p.status ?? (message.type==='task_result'?'completed':message.type==='task_error'?'failed':'running')); db.prepare('UPDATE agent_tasks SET status=?,updated_at=? WHERE id=?').run(status,now(),taskId); taskEvent(taskId,status,p); audit(String(task.user_id),'agent_task_event',{taskId,status}); }
  } catch { socket.send(JSON.stringify({type:'error',payload:{error:'invalid_message'}})); } });
  socket.on('close', () => { if (deviceId) { agentSockets.delete(deviceId); db.prepare("UPDATE devices SET status='offline' WHERE id=?").run(deviceId); } });
});
