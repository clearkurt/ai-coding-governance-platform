...
function renderGeneration(g: Awaited<ReturnType<typeof provider.generate>>) { return g.suggestion; }
...
app.post('/api/chat/stream', async (request, reply) => {
  ...
  const body = z.object({ conversationId:z.string().uuid(), content:z.string().min(1).max(10000), deviceId:z.string().uuid().optional(), rootId:z.string().uuid().optional(), sources:z.array(z.object({name:z.string(),content:z.string()})).max(20).default([]) }).safeParse(request.body);
  if (!body.success) return reply.code(400).send({ error: '聊天消息无效' });
  const bound = db.prepare('SELECT root_id AS rootId FROM conversations WHERE id=? AND user_id=?').get(body.data.conversationId, user.id) as { rootId: string | null } | undefined;
  if (!bound) return reply.code(404).send({ error: '会话不存在' });
  let deviceId = body.data.deviceId; let rootId = body.data.rootId;
  if (bound.rootId) {
    if (rootId && rootId !== bound.rootId) return reply.code(409).send({ error: '该对话已绑定项目，不能在对话中途切换项目' });
    const boundDevice = db.prepare('SELECT r.device_id AS deviceId FROM device_roots r JOIN devices d ON d.id=r.device_id WHERE r.id=? AND d.user_id=?').get(bound.rootId, user.id) as { deviceId: string } | undefined;
    if (!boundDevice) return reply.code(400).send({ error: '对话绑定的项目已不存在' });
    deviceId = boundDevice.deviceId; rootId = bound.rootId;
  }
  if (deviceId) {
    if (!db.prepare('SELECT 1 FROM devices WHERE id=? AND user_id=?').get(deviceId, user.id)) return reply.code(404).send({ error: '设备不存在' });
    if (!rootId || !db.prepare('SELECT 1 FROM device_roots WHERE id=? AND device_id=?').get(rootId, deviceId)) return reply.code(400).send({ error: '项目根目录无效' });
  }
  const projectRules = (rootId ? (db.prepare('SELECT rules FROM device_roots WHERE id=?').get(rootId) as { rules: string | null } | undefined)?.rules : undefined) ?? '';
  const agentProvider = provider as Partial<import('./model.js').AgentModelProvider>;
  if (typeof agentProvider.plan !== 'function') return reply.code(503).send({ error: '当前模型未配置智能聊天接口' });
  ...
  try {
    ...
    const generation = await runServerAgent(agentProvider as import('./model.js').AgentModelProvider, { requirement:body.data.content, codeStyle, sources:body.data.sources, history, toolsAvailable: Boolean(deviceId && rootId), projectRules }, async call => {
      const safeCall = validateAgentToolCall(call);
      if (!deviceId || !rootId) return { error: '未连接本地 Agent，无法使用文件工具，请直接回答用户' };
      const task = dispatchAgentTask(user.id, deviceId, rootId, safeCall.name, safeCall.arguments);
      return { taskId: task.taskId, ...(await task.result) };
    }, 8, (call,result) => { toolTrace.push({ name:call.name, result }); if (deviceId && rootId) send('tool', { name:call.name, result }); }, delta => send('token', { text: delta }));
    const content = generation.suggestion;
    db.prepare('INSERT INTO messages (id,conversation_id,role,content,metadata,created_at) VALUES (?,?,?,?,?,?)').run(id(), body.data.conversationId, 'assistant', content, JSON.stringify({ unified:true, toolTrace }), now());
    db.prepare('UPDATE conversations SET updated_at=? WHERE id=?').run(now(), body.data.conversationId);
    send('complete', { content, generation, toolTrace });
  } catch (error) { send('error', { error: error instanceof Error ? error.message : '聊天失败' }); } finally { send('done', {}); reply.raw.end(); }
});
...
app.post('/api/conversations', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const body = z.object({title:z.string().min(1).max(100),rootId:z.string().uuid().optional()}).safeParse(request.body); if (!body.success) return reply.code(400).send({error:'标题不能为空'}); if (body.data.rootId && !db.prepare('SELECT 1 FROM device_roots WHERE id=? AND device_id IN (SELECT id FROM devices WHERE user_id=?)').get(body.data.rootId,user.id)) return reply.code(400).send({error:'项目根目录无效'}); const conversation = { id:id(), title:body.data.title, rootId: body.data.rootId ?? null, createdAt:now() }; db.prepare('INSERT INTO conversations (id,user_id,title,root_id,created_at,updated_at) VALUES (?,?,?,?,?,?)').run(conversation.id,user.id,conversation.title,conversation.rootId,conversation.createdAt,conversation.createdAt); return { conversation }; });
app.get('/api/conversations', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; return { conversations: db.prepare('SELECT id,title,root_id AS rootId,created_at AS createdAt,updated_at AS updatedAt FROM conversations WHERE user_id=? ORDER BY updated_at DESC').all(user.id) }; });
app.get('/api/conversations/:id', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const conversation = db.prepare('SELECT id,title,root_id AS rootId,created_at AS createdAt FROM conversations WHERE id=? AND user_id=?').get((request.params as {id:string}).id,user.id); if (!conversation) return reply.code(404).send({error:'会话不存在'}); const messages = db.prepare('SELECT id,role,content,metadata,created_at AS createdAt FROM messages WHERE conversation_id=? ORDER BY created_at').all((conversation as {id:string}).id); return { conversation, messages }; });
app.delete('/api/conversations/:id', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const conversationId = (request.params as {id:string}).id; const existing = db.prepare('SELECT 1 FROM conversations WHERE id=? AND user_id=?').get(conversationId, user.id); if (!existing) return reply.code(404).send({error:'会话不存在'}); db.prepare('DELETE FROM messages WHERE conversation_id=?').run(conversationId); db.prepare('DELETE FROM conversations WHERE id=? AND user_id=?').run(conversationId, user.id); audit(user.id,'conversation_deleted',{conversationId}); return { ok: true }; });
...
app.post('/api/agent-runs', async (request, reply) => {
  ...
  try {
    ...
    const generation=await runServerAgent(agentProvider as import('./model.js').AgentModelProvider,{requirement:body.data.requirement,codeStyle,sources:body.data.sources},async call=>{
      if (!['list_files','read_file','stage_patch','apply_patch','run_command'].includes(call.name)) throw new Error('不支持的 Agent 工具');
      const safeCall = validateAgentToolCall(call);
...
app.get('/api/agents', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const devices = db.prepare('SELECT id,name,version,status,last_seen_at AS lastSeenAt,created_at AS createdAt FROM devices WHERE user_id=? ORDER BY created_at DESC').all(user.id) as Array<Record<string, unknown>>; return { devices: devices.map(device => ({ ...device, roots: db.prepare('SELECT id,label,rules FROM device_roots WHERE device_id=?').all(String(device.id)) })) }; });
app.get('/api/agents/:id', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const device = db.prepare('SELECT id,name,version,status,last_seen_at AS lastSeenAt,created_at AS createdAt FROM devices WHERE id=? AND user_id=?').get((request.params as {id:string}).id, user.id); if (!device) return reply.code(404).send({ error: '设备不存在' }); return { device, roots: db.prepare('SELECT id,label,rules FROM device_roots WHERE device_id=?').all((device as {id:string}).id) }; });
app.post('/api/agents/:id/tasks', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const deviceId = (request.params as {id:string}).id; if (!db.prepare('SELECT 1 FROM devices WHERE id=? AND user_id=?').get(deviceId,user.id)) return reply.code(404).send({error:'设备不存在'}); const parsed = z.object({ rootId:z.string().uuid(), kind:z.enum(['list_files','read_file','stage_patch','run_command','select_root']), payload:z.record(z.unknown()) }).safeParse(request.body); if (!parsed.success) return reply.code(400).send({error:'任务参数无效'}); if (!db.prepare('SELECT 1 FROM device_roots WHERE id=? AND device_id=?').get(parsed.data.rootId,deviceId)) return reply.code(400).send({error:'项目根目录无效'}); if (parsed.data.kind==='select_root') { sendPendingTasks(deviceId); if (db.prepare("SELECT 1 FROM agent_tasks WHERE device_id=? AND root_id=? AND kind='select_root' AND status IN ('queued','dispatched','running','awaiting_approval')").get(deviceId,parsed.data.rootId)) return reply.code(409).send({error:'该目录已有更换任务正在等待 Agent 处理，请稍后再试'}); } const taskId=id(); const expiresAt=new Date(Date.now()+30*60_000).toISOString(); db.prepare('INSERT INTO agent_tasks (id,device_id,user_id,root_id,kind,payload,status,expires_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(taskId,deviceId,user.id,parsed.data.rootId,parsed.data.kind,JSON.stringify({...parsed.data.payload,rootId:parsed.data.rootId,kind:parsed.data.kind}),'queued',expiresAt,now(),now()); taskEvent(taskId,'queued',{}); audit(user.id,'agent_task_created',{taskId,deviceId,kind:parsed.data.kind}); sendPendingTasks(deviceId); return { taskId, status:'queued', expiresAt }; });
...
app.post('/api/agent-tasks/:id/approve', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const taskId=(request.params as {id:string}).id; const task=db.prepare("SELECT * FROM agent_tasks WHERE id=? AND user_id=? AND kind IN ('stage_patch','run_command') AND status='awaiting_approval' AND expires_at>?").get(taskId,user.id,now()) as Record<string,unknown>|undefined; if (!task) return reply.code(409).send({error:'没有可批准的补丁或命令任务'}); const token=randomBytes(24).toString('base64url'); const kind=String(task.kind); const original=JSON.parse(String(task.payload)) as Record<string,unknown>; const payload=kind==='stage_patch' ? {...original, approvalToken:token, approvalTokenHash:sha(token), kind:'apply_patch'} : {...original, approvalToken:token, approvalTokenHash:sha(token)}; db.prepare("UPDATE agent_tasks SET status='approved',approval_token_hash=?,payload=?,updated_at=? WHERE id=?").run(sha(token),JSON.stringify(payload),now(),taskId); taskEvent(taskId,'approved',{}); audit(user.id,kind==='stage_patch'?'agent_patch_approved':'agent_command_approved',{taskId}); sendPendingTasks(String(task.device_id)); return { taskId,status:'approved' }; });
app.post('/api/agent-tasks/:id/reject', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const taskId = (request.params as { id: string }).id; const task = db.prepare("SELECT * FROM agent_tasks WHERE id=? AND user_id=? AND kind IN ('stage_patch','run_command') AND status='awaiting_approval' AND expires_at>?").get(taskId, user.id, now()) as Record<string, unknown> | undefined; if (!task) return reply.code(409).send({ error: '没有可拒绝的补丁或命令任务' }); const kind = String(task.kind); db.prepare("UPDATE agent_tasks SET status='rejected',updated_at=? WHERE id=?").run(now(), taskId); taskEvent(taskId, 'rejected', {}); audit(user.id, kind==='stage_patch' ? 'agent_patch_rejected' : 'agent_command_rejected', { taskId }); agentTaskBridge.complete(taskId, { status: 'rejected' }); return { taskId, status: 'rejected' }; });
...
app.get('/api/agent/ws', { websocket: true }, (socket) => {
  ...
  socket.on('message', (raw: Buffer) => { try { const message=JSON.parse(raw.toString()) as {type:string;payload?:Record<string,unknown>;taskId?:string;deviceId?:string};
    ...
    if (message.type==='hello') {
      const requestedDevice=String(message.deviceId ?? ''); const credential=String(p.credential ?? '');
      const device=db.prepare('SELECT * FROM devices WHERE id=? AND credential_hash=?').get(requestedDevice,sha(credential)) as Record<string,unknown>|undefined;
      if (!device) return socket.close();
      deviceId=requestedDevice; agentSockets.set(deviceId,socket);
      db.prepare("UPDATE devices SET status='online',last_seen_at=?,version=? WHERE id=?").run(now(),String(p.version ?? device.version),deviceId);
      if (Array.isArray(p.rules)) {
        for (const entry of p.rules as Array<Record<string, unknown>>) {
          const rootId = String(entry.rootId ?? ''); const content = String(entry.content ?? '');
          if (rootId && content) db.prepare('UPDATE device_roots SET rules=? WHERE id=? AND device_id=?').run(content, rootId, deviceId);
        }
      }
      socket.send(JSON.stringify({version:1,messageId:id(),type:'hello_result',deviceId,payload:{protocol:1}})); sendPendingTasks(deviceId); return;
    }
    if (!deviceId || String(message.deviceId ?? '')!==deviceId) return;
    if (message.type==='heartbeat') { db.prepare("UPDATE devices SET status='online',last_seen_at=? WHERE id=?").run(now(),deviceId); return; }
    if (message.type==='task_status' || message.type==='task_result' || message.type==='task_error') { const taskId=String(message.taskId ?? ''); const task=db.prepare('SELECT * FROM agent_tasks WHERE id=? AND device_id=?').get(taskId,deviceId) as Record<string,unknown>|undefined; if (!task) return; const status=String(p.status ?? (message.type==='task_result'?'completed':message.type==='task_error'?'failed':'running')); if (String(task.kind)==='select_root' && status==='completed') { const result=(p.result ?? {}) as Record<string,unknown>; const label=String(result.label ?? '项目目录'); db.prepare('UPDATE device_roots SET label=? WHERE id=? AND device_id=?').run(label,String(task.root_id),deviceId); if (typeof result.rules === 'string') db.prepare('UPDATE device_roots SET rules=? WHERE id=? AND device_id=?').run(result.rules,String(task.root_id),deviceId); } db.prepare('UPDATE agent_tasks SET status=?,updated_at=? WHERE id=?').run(status,now(),taskId); const eventDetail={ ...p }; delete eventDetail.credential; taskEvent(taskId,status,eventDetail); audit(String(task.user_id),'agent_task_event',{taskId,status}); if (['completed','failed','rejected','expired','awaiting_approval'].includes(status)) agentTaskBridge.complete(taskId,{status,result:p.result,error:typeof p.error==='string'?p.error:undefined}); }
