...
app.post('/api/agents/:id/tasks', async (request, reply) => { const user = requireUser(request, reply); if (!user) return; const deviceId = (request.params as {id:string}).id; if (!db.prepare('SELECT 1 FROM devices WHERE id=? AND user_id=?').get(deviceId,user.id)) return reply.code(404).send({error:'设备不存在'}); const parsed = z.object({ rootId:z.string().uuid(), kind:z.enum(['list_files','read_file','stage_patch','select_root']), payload:z.record(z.unknown()) }).safeParse(request.body); if (!parsed.success) return reply.code(400).send({error:'任务参数无效'}); if (!db.prepare('SELECT 1 FROM device_roots WHERE id=? AND device_id=?').get(parsed.data.rootId,deviceId)) return reply.code(400).send({error:'项目根目录无效'}); const taskId=id(); const expiresAt=new Date(Date.now()+10*60_000).toISOString(); db.prepare('INSERT INTO agent_tasks (id,device_id,user_id,root_id,kind,payload,status,expires_at,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)').run(taskId,deviceId,user.id,parsed.data.rootId,parsed.data.kind,JSON.stringify({...parsed.data.payload,rootId:parsed.data.rootId,kind:parsed.data.kind}),'queued',expiresAt,now(),now()); taskEvent(taskId,'queued',{}); audit(user.id,'agent_task_created',{taskId,deviceId,kind:parsed.data.kind}); sendPendingTasks(deviceId); return { taskId, status:'queued', expiresAt }; });
