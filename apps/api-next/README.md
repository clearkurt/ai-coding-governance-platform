...
The real-network model proxy integration starts the central API and a DeepSeek-like localhost HTTPS upstream as separate uvicorn processes. The central process trusts only the temporary CA, injects the server-only provider key, transparently streams the documented Responses events, and records provider usage exactly once in PostgreSQL. Missing, expired, and terminal-task-revoked model tokens are rejected before the upstream request count changes. This proves the TLS/HTTP proxy boundary, not DeepSeek API behavior or model quality.
