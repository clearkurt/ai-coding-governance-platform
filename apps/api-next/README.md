...
The current acceptance baseline is 35 passing regular pytest tests, five environment-gated PostgreSQL integration tests, and a clean Ruff check. The migration lifecycle, real `PostgresStore` lifecycle, same-process localhost WebSocket reconnect, and subprocess uvicorn restart integrations have all been run successfully against a local PostgreSQL 16 instance. The custom-format backup/restore drill is implemented but still requires an explicit run where PostgreSQL 16 client tools and an administrative test URL are available. Real DeepSeek Responses traffic and production TLS/WSS remain release gates; see [rollout-acceptance.md](../../docs/rollout-acceptance.md).
