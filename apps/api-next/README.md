...
The current acceptance baseline is 30 passing regular pytest tests, four environment-gated PostgreSQL integration tests, and a clean Ruff check. The migration lifecycle, real `PostgresStore` lifecycle, and same-process localhost WebSocket reconnect integrations have all been run successfully against a local PostgreSQL 16 instance. The new subprocess uvicorn restart integration awaits an explicit run. Real DeepSeek Responses traffic and production TLS/WSS remain release gates; see [rollout-acceptance.md](../../docs/rollout-acceptance.md).
