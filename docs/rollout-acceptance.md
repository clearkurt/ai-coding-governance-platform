...
| 范围 | 结果 |
|---|---|
| FastAPI | 普通 `pytest` 88 项通过、7 项真实 PostgreSQL integration 默认 skipped；migration、`PostgresStore` lifecycle、同进程 localhost WebSocket reconnect、uvicorn 子进程重启、custom-format backup/restore、localhost TLS/WSS 与灰度回滚恢复 integration 均已在本机 PostgreSQL 16 显式运行通过；`ruff check .` 通过 |
