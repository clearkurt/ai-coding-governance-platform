...
| 范围 | 结果 |
|---|---|
| FastAPI | 普通 `pytest` 30 项通过、3 项真实 PostgreSQL integration 默认 skipped；migration、`PostgresStore` lifecycle 与 localhost WebSocket reconnect integration 均已在本机 PostgreSQL 16 显式运行通过；`ruff check .` 通过 |
